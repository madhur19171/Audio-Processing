...
int main()
{
   ...
   /*for (int j = 1; j < length + 1; j++)
      for (int i = 0; i < rate; i++)
         writ.push_back((short)(10000 * sin(2.0 * pi * frequency * (i / rate))));
         */
   for (int j = 1; j <= length * frequency; j++)
   {
      for (int i = 1; i <= (rate / frequency) / 2; i++)
         square.push_back((short)(10000));
      for (int i = 1; i <= (rate / frequency) / 2; i++)
         square.push_back((short)(-10000));
   }

   char name1[] = "square.wav";
   WaveIO obj1(name1, 1, rate, 16, square);

   vector<short> sine;
   for (int j = 1; j < length + 1; j++)
      for (int i = 0; i < rate; i++)
         sine.push_back((short)(10000 * sin(2.0 * pi * frequency * (i / rate))));

   char name2[] = "sine.wav";
   WaveIO obj2(name2, 1, rate, 16, sine);

   vector<short> saw;
   for (int j = 1; j <= length * frequency; j++)
   {
      for (int i = 1; i <= (rate / frequency) / 2; i++)
         saw.push_back((short)(1000 * i));
      for (int i = (rate / frequency) / 2; i >= 1; i--)
         saw.push_back((short)(-1000 * i));
   }

   char name3[] = "saw.wav";
   WaveIO obj3(name3, 1, rate, 16, saw);
}